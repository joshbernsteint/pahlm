...
const mathMacros = [
    //Superscript
    {
        ...
        run: (flags, s, g1, g2) => {
            return `<msup>${flags.math.parser(g1, flags)}${flags.math.parser(g2, flags)}</msup>`
        }
    },
    //Subscript
    {
        ...
        run: (flags, s, g1, g2) => {
            return `<msub>${flags.math.parser(g1, flags)}${flags.math.parser(g2, flags)}</msub>`
        }
    },
    //Text
    {
        pattern: [/\\text/gm, ["{","}"]],
        run: (flags, s, g1) => flags.customCommands.parser(flags, g1)
    },
    //Square root
    {
        pattern: [/\\sqrt/gm, ["{","}"]],
        run: (flags, s, g1) => `<msqrt>${flags.math.parser(g1,flags)}</msqrt>`
    },
    //Root
    {
        pattern: [/\\root/gm, ["{","}"], ["{","}"]],
        run: (flags, s, g1, g2) => `<mroot>${flags.math.parser(g1,flags)}${flags.math.parser(g2,flags)}</mroot>`
    },
    //Fraction
    {
        pattern: [/\\frac/gm, ["{","}"], ["{","}"]],
        run: (flags, s, g1, g2) => `<mfrac>${flags.math.parser(g1,flags)}${flags.math.parser(g2,flags)}</mfrac>`
    },    
    //Binomial
    {
        pattern: [/\\binom/gm, ["{","}"], ["{","}"]],
        run: (flags, s, g1, g2) => `<mo>&lpar;</mo><mfrac linethickness="0">${flags.math.parser(g1,flags)}${flags.math.parser(g2,flags)}</mfrac><mo>&rpar;</mo>`
    },
    //Integral
    {
        pattern: [/\\int/gm, ["{","}"], ["{","}"]],
        run: (flags, s, g1, g2) => `<msubsup><mo>&#x222B;</mo>${flags.math.parser(g1,flags)}${flags.math.parser(g2,flags)}</msubsup>`
    },
